...
const OrderItem = ( {order} : OrderItemProps) => {
    ...
    const total = useMemo(() =>{
        ...
        return order.orderProducts.reduce((acc, orderProduct) =>{
            ...
            return(
                acc + productsWithTotalPrice.totalPrice * orderProduct.quantity
            );
        }, 0 );
    },[order.orderProducts])
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between lg:hidden">
                                <div className="font-bold">
                                    <p className="text-xs lg:text-sm">Status</p>
                                    <p className="text-xs text-[#8162FF] lg:text-sm">{getOrderStatus(order.status)}</p>
                                </div>
                                
                                <div>
                                    <p className="text-xs font-bold lg:text-sm">Data</p>
                                    <p className="text-xs opacity-60 lg:text-sm">{format(order.createdAt, "d/MM/y")}</p>
                                </div>
